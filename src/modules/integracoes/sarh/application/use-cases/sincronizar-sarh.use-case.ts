import type { SarhEndpointKey, SarhResumoExecucao } from "../../domain/sarh.types";
import { endpointDbFromKey } from "../../domain/sarh-normalizer";
import { ENDPOINTS_PADRAO_SARH, type SincronizarSarhInput } from "../sarh-sync.dto";
...
export class SincronizarSarhUseCase {
  ...
  async execute(input: SincronizarSarhInput = {}): Promise<SarhResumoExecucao> {
    ...
    const tipo = input.tipo ?? (input.modoSimulacao ? "SIMULACAO" : "SINCRONIZACAO_COMPLETA");
    const modoSimulacao = input.modoSimulacao ?? true;
    const endpoints = input.endpoints?.length ? input.endpoints : ENDPOINTS_PADRAO_SARH;
    ...
    try {
      if (endpoints.includes("empresas")) {
        ...
        for (const empresa of empresas) {
          const bruto = await repository.registrarPayloadBruto({ execucaoId: execucao.id, endpoint: "empresas", payload: empresa });
      ...
      if (endpoints.includes("lotacoes")) {
        ...
        const filtradas = input.codigoUnidadeSarh
          ? lotacoes.filter((lotacao) => lotacao.id === input.codigoUnidadeSarh || lotacao.idPai === input.codigoUnidadeSarh)
          : lotacoes;

        for (const lotacao of filtradas) {
          const bruto = await repository.registrarPayloadBruto({ execucaoId: execucao.id, endpoint: "lotacoes", payload: lotacao });
      ...
      if (endpoints.includes("cargos")) {
        ...
        for (const cargo of filtrados) {
          const bruto = await repository.registrarPayloadBruto({ execucaoId: execucao.id, endpoint: "cargos", payload: cargo });
      ...
      if (endpoints.includes("servidores")) {
        ...
        const filtrados = input.matricula
          ? servidores.filter((servidor) => servidor.matricula.toUpperCase() === input.matricula?.toUpperCase())
          : servidores;

        for (const servidor of filtrados) {
          const bruto = await repository.registrarPayloadBruto({ execucaoId: execucao.id, endpoint: "servidores", payload: servidor });
      ...
      if (endpoints.includes("lotacoesServidores")) {
        const lotacoesServidores = await this.sarhClient.buscarLotacoesServidores();
        const filtradas = input.matricula
          ? lotacoesServidores.filter((item) => item.matricula.toUpperCase() === input.matricula?.toUpperCase())
          : lotacoesServidores;
      ...
      await repository.finalizarExecucao({ execucaoId: execucao.id, iniciadoEm, contadores });
      await repository.registrarLog({
        integracaoId: integracao.id,
        status: contadores.totalErros > 0 || contadores.totalConflitos > 0 ? "ERRO" : "SUCESSO",
        mensagem: `Sincronização SARH finalizada. Modo simulação: ${modoSimulacao ? "sim" : "não"}.`,
        payloadEntrada: input,
        payloadSaida: contadores,
        metadados: { execucaoId: execucao.id, endpoints: endpoints.map(endpointDbFromKey) },
        iniciadoEm,
      });
      ...
      await repository.finalizarExecucao({ execucaoId: execucao.id, iniciadoEm, contadores, erro: mensagem });
      await repository.registrarLog({
        integracaoId: integracao.id,
        status: "ERRO",
        ...
        erro: mensagem,
        payloadEntrada: input,
        metadados: { execucaoId: execucao.id },
        iniciadoEm,
      });
      throw error;
    }
