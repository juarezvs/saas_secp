...
export class SarhPrismaRepository {
  ...
  async obterOuCriarIntegracaoSarh() {
    const existente = await this.prisma.integracaoSistema.findFirst({
      where: { tipo: "SARH", nome: "SARH - Sistema de Gestão de Recursos Humanos" },
    });
    ...
    return this.prisma.integracaoSistema.create({
      data: {
        ...
        status: "ATIVA",
        direcao: "ENTRADA",
        baseUrl: process.env.SARH_BASE_URL ?? "http://sarh.integracao.am.trf1.gov.br",
        ativo: true,
        descricao:
  ...
  async registrarLog(params: {
    ...
    return this.prisma.logIntegracao.create({
      data: {
        ...
        erro: params.erro,
        payloadEntrada: params.payloadEntrada,
        payloadSaida: params.payloadSaida,
        metadados: params.metadados ?? {},
        iniciadoEm,
        finalizadoEm,
  ...
  async registrarItem(execucaoId: string, endpoint: TipoEndpointSarhDb, item: ResultadoItemSarh, registroBrutoId?: string | null) {
  ...
  async processarCargo(params: { execucaoId: string; payload: SarhCargoDto; modoSimulacao: boolean; registroBrutoId?: string | null }) {
    const endpoint: TipoEndpointSarhDb = "CARGOS";
    const chaveExterna = String(params.payload.id);
    const existente = await this.prisma.cargo.findUnique({ where: { codigoExternoSarh: params.payload.id } });
    ...
    if (params.modoSimulacao) {
      await this.registrarItem(params.execucaoId, endpoint, {
        tipoRegistro: "CARGO",
        chaveExterna,
        operacao,
        status: "PROCESSADO",
        ...
        mensagem: `Simulação: cargo seria ${existente ? "atualizado" : "criado"}.`,
    ...
    await this.upsertMapeamento("CARGO", chaveExterna, "Cargo", salvo.id, gerarHashRegistro(params.payload));
    await this.registrarItem(params.execucaoId, endpoint, {
      tipoRegistro: "CARGO",
      chaveExterna,
      operacao,
  ...
  async processarEmpresa(params: { execucaoId: string; payload: SarhEmpresaDto; modoSimulacao: boolean; registroBrutoId?: string | null }) {
    const endpoint: TipoEndpointSarhDb = "EMPRESAS";
    const chaveExterna = String(params.payload.id);
    const sigla = limparTexto(params.payload.sigla) ?? `SARH-${params.payload.id}`;
    ...
    if (params.modoSimulacao) {
      await this.registrarItem(params.execucaoId, endpoint, {
        ...
        entidadeInternaId: existente?.id,
        mensagem: `Simulação: órgão ${sigla} seria ${existente ? "atualizado" : "criado"}.`,
        dadosAntes: existente,
        dadosDepois: data,
        metadados: { modoSimulacao: true },
      }, params.registroBrutoId);
    ...
    await this.upsertMapeamento("EMPRESA", chaveExterna, "Orgao", salvo.id, gerarHashRegistro(params.payload));
    await this.registrarItem(params.execucaoId, endpoint, {
      tipoRegistro: "EMPRESA",
      chaveExterna,
      operacao,
  ...
  async processarUnidade(params: { execucaoId: string; payload: SarhLotacaoDto; modoSimulacao: boolean; registroBrutoId?: string | null }) {
    ...
    const pai = params.payload.idPai
      ? await this.prisma.unidadeOrganizacional.findFirst({
          where: { orgaoId: orgao.id, codigoExternoSarh: params.payload.idPai },
        })
      : null;
    ...
    if (params.modoSimulacao) {
      await this.registrarItem(params.execucaoId, endpoint, {
        ...
        entidadeInternaId: existente?.id,
        mensagem: `Simulação: unidade ${data.sigla} seria ${existente ? "atualizada" : "criada"}.`,
        dadosAntes: existente,
        dadosDepois: data,
    ...
    const salvo = existente
      ? await this.prisma.unidadeOrganizacional.update({ where: { id: existente.id }, data })
      : await this.prisma.unidadeOrganizacional.create({ data });

    await this.upsertMapeamento("LOTACAO", chaveExterna, "UnidadeOrganizacional", salvo.id, gerarHashRegistro(params.payload));
    await this.registrarItem(params.execucaoId, endpoint, {
      tipoRegistro: "LOTACAO",
      chaveExterna,
      operacao,
      ...
      dadosAntes: existente,
      dadosDepois: salvo,
      metadados: { paiEncontrado: Boolean(pai), idPaiSarh: params.payload.idPai },
    }, params.registroBrutoId);
  ...
  async corrigirPaisDasUnidadesSarh() {
    ...
    const unidadesSemPai = await this.prisma.unidadeOrganizacional.findMany({
      ...
      select: { id: true, codigoExternoSarh: true, codigoExternoPaiSarh: true, unidadePaiId: true },
    });

    for (const unidade of unidadesSemPai) {
      if (!unidade.codigoExternoPaiSarh || unidade.codigoExternoPaiSarh === 4) continue;
      const pai = await this.prisma.unidadeOrganizacional.findFirst({
        where: { orgaoId: orgao.id, codigoExternoSarh: unidade.codigoExternoPaiSarh },
        select: { id: true },
      });
      if (pai && unidade.unidadePaiId !== pai.id) {
        await this.prisma.unidadeOrganizacional.update({ where: { id: unidade.id }, data: { unidadePaiId: pai.id } });
      }
    }
  }

  async processarServidor(params: { execucaoId: string; payload: SarhServidorDto; modoSimulacao: boolean; registroBrutoId?: string | null }) {
    ...
    const cargo = params.payload.cargoId
      ? await this.prisma.cargo.findUnique({ where: { codigoExternoSarh: params.payload.cargoId } })
      : null;

    const usuarioExistente = await this.prisma.usuario.findUnique({ where: { matricula } });
    const servidorExistente = await this.prisma.servidor.findUnique({ where: { matricula } });
    const usuarioData = mapearUsuarioServidorSarh(params.payload);
    const servidorBaseData = mapearServidorSarh(params.payload, usuarioExistente?.id ?? "__USUARIO_A_CRIAR__", orgao.id, cargo?.id ?? null);
    const operacao: OperacaoRegistroSarhDb = servidorExistente ? "ATUALIZAR" : "CRIAR";
    ...
    if (params.modoSimulacao) {
      await this.registrarItem(params.execucaoId, endpoint, {
        ...
        mensagem: `Simulação: servidor ${matricula} seria ${servidorExistente ? "atualizado" : "criado"}.`,
        dadosAntes: { usuario: usuarioExistente, servidor: servidorExistente },
        dadosDepois: { usuario: usuarioData, servidor: servidorBaseData },
        metadados: { modoSimulacao: true, cargoEncontrado: Boolean(cargo) },
      }, params.registroBrutoId);
      return operacao;
    }

    const usuario = usuarioExistente
      ? await this.prisma.usuario.update({ where: { id: usuarioExistente.id }, data: usuarioData })
      : await this.prisma.usuario.create({ data: usuarioData });

    const servidorData = mapearServidorSarh(params.payload, usuario.id, orgao.id, cargo?.id ?? null);
    const servidor = servidorExistente
      ? await this.prisma.servidor.update({ where: { id: servidorExistente.id }, data: servidorData })
      : await this.prisma.servidor.create({ data: servidorData });

    await this.vincularPerfilServidor(usuario.id);
    await this.upsertMapeamento("SERVIDOR", chaveExterna, "Servidor", servidor.id, gerarHashRegistro(params.payload));
    await this.registrarItem(params.execucaoId, endpoint, {
      tipoRegistro: "SERVIDOR",
      chaveExterna,
      operacao,
      ...
      dadosAntes: { usuario: usuarioExistente, servidor: servidorExistente },
      dadosDepois: { usuario, servidor },
      metadados: { cargoEncontrado: Boolean(cargo), cargoIdSarh: params.payload.cargoId },
    }, params.registroBrutoId);

    return operacao;
  }

  async processarLotacaoServidor(params: { execucaoId: string; payload: SarhLotacaoServidorDto; modoSimulacao: boolean; registroBrutoId?: string | null }) {
    const endpoint: TipoEndpointSarhDb = "LOTACOES_SERVIDORES";
    const matricula = normalizarMatricula(params.payload.matricula);
    const chaveExterna = `${matricula}:${params.payload.lotacaoId ?? "sem-lotacao"}`;
    const servidor = await this.prisma.servidor.findUnique({ where: { matricula }, include: { usuario: true } });
    ...
    if (!servidor) {
      await this.registrarItem(params.execucaoId, endpoint, {
        ...
        metadados: { matricula, lotacaoId: params.payload.lotacaoId },
      }, params.registroBrutoId);
    ...
    if (isLotacaoServidorDesligado(params.payload)) {
      if (!params.modoSimulacao) {
        await this.prisma.usuario.update({ where: { id: servidor.usuarioId }, data: { ativo: false } });
        await this.prisma.servidor.update({ where: { id: servidor.id }, data: { ativo: false } });
        await this.prisma.lotacao.updateMany({
          where: { servidorId: servidor.id, status: "ATIVO" },
          data: { status: "INATIVO", dataFim: new Date() },
        });
      }
    ...
    const lotacaoAtiva = await this.prisma.lotacao.findFirst({
      where: { servidorId: servidor.id, status: "ATIVO", tipo: "TITULAR" },
    });

    const mesmaLotacao = lotacaoAtiva?.unidadeId === unidade.id;
    const operacao: OperacaoRegistroSarhDb = lotacaoAtiva ? (mesmaLotacao ? "ATUALIZAR" : "ATUALIZAR") : "CRIAR";
    ...
    if (params.modoSimulacao) {
      await this.registrarItem(params.execucaoId, endpoint, {
        ...
        dadosDepois: { servidorId: servidor.id, unidadeId: unidade.id, cargoId: cargo?.id ?? null },
        metadados: { modoSimulacao: true },
      }, params.registroBrutoId);
      return operacao;
    }
    ...
    await this.upsertMapeamento("LOTACAO_SERVIDOR", chaveExterna, "Lotacao", lotacaoSalva.id, gerarHashRegistro(params.payload));
    await this.registrarItem(params.execucaoId, endpoint, {
      tipoRegistro: "LOTACAO_SERVIDOR",
      chaveExterna,
      operacao,
  ...
  incrementar(contadores: ContadoresExecucao, operacao: OperacaoRegistroSarhDb) {
  ...
  private async obterOrgaoPadrao() {
    ...
    const orgao = await this.prisma.orgao.findFirst({
      where: { OR: [{ codigoExternoSarh: codigoSarh }, { sigla }] },
    });
    ...
    if (!orgao.codigoExternoSarh) {
      return this.prisma.orgao.update({ where: { id: orgao.id }, data: { codigoExternoSarh: codigoSarh } });
    }
  ...
  private async vincularPerfilServidor(usuarioId: string) {
    const perfil = await this.prisma.perfil.findUnique({ where: { codigo: "SERVIDOR" } });
    if (!perfil) return;

    await this.prisma.usuarioPerfil.upsert({
      where: { usuarioId_perfilId: { usuarioId, perfilId: perfil.id } },
      update: { ativo: true },
      create: { usuarioId, perfilId: perfil.id, ativo: true },
    });
  }
  ...
  private async upsertMapeamento(
    ...
    return this.prisma.mapeamentoExterno.upsert({
      ...
      update: { entidadeInterna, entidadeInternaId, hashAtual, ativo: true },
      create: {
