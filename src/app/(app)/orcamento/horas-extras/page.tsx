...
import { listarSolicitacoesHorasExtrasParaOrcamento } from "@/modules/horas-extras/infrastructure/repositories/horas-extras-solicitacao.repository";
...
export default async function OrcamentoHorasExtrasPage() {
  const permissao = await exigirPermissaoOuRedirecionar(
    "horas-extras:responder-orcamento:global",
  );
  let solicitacoes: Awaited<
    ReturnType<typeof listarSolicitacoesHorasExtrasParaOrcamento>
  > = [];
  let erroConsulta: string | null = null;

  try {
    solicitacoes = await listarSolicitacoesHorasExtrasParaOrcamento({
    ...
    erroConsulta =
      error instanceof Error
        ? error.message
        : "Nao foi possivel consultar as solicitacoes.";
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader
        icon={Landmark}
        titulo="Orçamento de horas extras"
        descricao="Solicitações aguardando parecer orçamentário."
      />

      <Card>
        <CardHeader>
          <CardTitle>Pendentes de parecer</CardTitle>
        </CardHeader>
        <CardContent>
          {erroConsulta ? (
            ...
            <div className="divide-y rounded-md border">
              {solicitacoes.map((solicitacao) => {
                ...
                return (
                  <Link
                    key={solicitacao.id}
                    href={`/orcamento/horas-extras/${solicitacao.id}`}
            ...
            <p className="text-sm text-muted-foreground">
              Nenhuma solicitação aguardando parecer orçamentário.
            </p>
          )}
        </CardContent>
      </Card>
