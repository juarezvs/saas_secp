...
type MinhaEquipePresencasPageProps = {
...
export default async function MinhaEquipePresencasPage({
  searchParams,
}: MinhaEquipePresencasPageProps) {
  ...
  const idsSubordinados =
    !visualizarTodasEquipes && permissao.usuarioId
      ...
      : [];
  const dados = permissao.usuarioId
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader
        icon={UsersRound}
        titulo="Presentes/Ausentes/Licenças"
        descricao={
          visualizarTodasEquipes
      ...
      <MinhaEquipeFiltros
        data={data}
        anoFerias={new Date().getFullYear()}
        escopo={dados.escopo}
        resumo={dados.resumo}
